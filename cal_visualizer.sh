...
### STATUS WALKER
status_task() {
    # Evaluate which status should be set
    PRE_STATUS=$(grep STATUS $root/mod_task.ics | cut -d':' -f2) # Remove carriage return character from PRE_STATUS
    PRE_STATUS=$(echo $PRE_STATUS | tr -d '\r') # Change status
    case "$PRE_STATUS" in
        NEEDS-ACTION)
    ...
    #Mod the task
    sed -i "s/^STATUS:.*/STATUS:$STATUS_TO_SET/" $root/mod_task.ics
...
line_number=$(echo $SELECTED_TASK | cut -d'.' -f1 | grep -n -f - $root/tasks | cut -d: -f1) # Find the task from the summary
